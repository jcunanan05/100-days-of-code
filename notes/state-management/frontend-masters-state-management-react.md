...
- general tip is to clean up render method as possible, so it's easier to read. remember: **_You'll spend more time reading code than writing code so save it for the future you_**
...
- Actions - plain old javascript objects that is like the user's actions but in state.

  - e.g. `{ type: 'UPDATE_COUNT', value: 0 }`

- Action Creators - Function that dispatches actions
  - e.g.

```javascript
export const updateCount = value => {
  AppDispatcher.dispatch({type: 'UPDATE_COUNT' value});
};
```

- Store - the thing that holds the state of your application

  - > Tip: It's easier to what to name store as, _"what are the nouns in your application?"_, it will likely have a store.
  - e.g. _Current User_, _Notifications_
  - The store will emit an event after it is updated. (Using nodejs `EventEmitter` class) So all the views can listen in case anythings changed in the store.
